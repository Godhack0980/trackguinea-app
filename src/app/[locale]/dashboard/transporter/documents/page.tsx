...
const DocumentManager = ({ 
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      // Use uploadBytesResumable to track progress and handle errors immediately
      ...
      try {
        ...
        const result = (await verifyDocument({
          url: downloadURL,
          docKey,
          selectedDocSubType: docKey === 'identityCard' ? selectedDocSubType : undefined,
        ...
        if (result.isValid && docKey === 'identityCard') {
        ...
        await createNotification({
          ...
          href: '/dashboard/transporter/documents'
        });
        ...
        adminSnapshot.forEach(async (adminDoc) => {
          await createNotification({
            userId: adminDoc.id,
            message: result.isValid 
              ? `Le transporteur ${userName} a validé son document "${title}" via notre système.` 
              : `Le transporteur ${userName} a eu son document "${title}" refusé par notre système.`,
        ...
        if (result.isValid) {
          toast({ 
            ...
            description: `Les informations du document ont été analysées et validées par notre système.`
          });
        } else {
          toast({ 
            variant: "destructive", 
  ...
  const onInfoSubmit = async (values: DocumentFormData) => {
    if (docKey === 'identityCard' && !selectedDocSubType) {
      setSubTypeError(true);
      return;
    }

    // Validate passport number rules
    if (docKey === 'identityCard' && selectedDocSubType === 'passport') {
    ...
    try {
      ...
      if (docKey === 'identityCard') {
        updateData[`documents.${docKey}.subType`] = selectedDocSubType;
      }
      await updateDoc(userDocRef, updateData);
  ...
  const getValidityProgress = () => {
    if (!docInfo?.expiryDate || !docInfo?.issueDate) return null;
    const start = docInfo.issueDate.toDate().getTime();
  ...
  return (
    <Card className={cn(
      ...
      <CardHeader className="pb-3 border-b border-border/20 bg-muted/20">
        ...
        <CardDescription className="text-xs mt-1">Renseignez les dates de validité et ajoutez le document scanné.</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-5 pt-5">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onInfoSubmit)} className="space-y-4">
            ...
            <FormField control={form.control} name="docNumber" render={({ field }) => (
              <FormItem>
                <FormLabel className="text-xs font-bold text-muted-foreground">Numéro de Document</FormLabel>
                <FormControl>
                  <Input 
                    {...field} 
                    className="rounded-xl border-border/50 bg-background" 
                    placeholder={
                      docKey === 'identityCard' && selectedDocSubType === 'passport' 
                        ? 'Ex: GP1234567 (9 caractères) ou NIN (15 chiffres)' 
                        : docKey === 'identityCard' && selectedDocSubType === 'identityCard' 
                        ? 'Ex: 1234567890123456 (16 chiffres)' 
                        : 'Ex: N° de document'
                    } 
                  />
                </FormControl>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <FormField control={form.control} name="issueDate" render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel className="text-xs font-bold text-muted-foreground mb-1">Date d'Émission</FormLabel>
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button variant={"outline"} className={cn("rounded-xl border-border/50 bg-background text-left font-normal", !field.value && "text-muted-foreground")}>
                          {field.value ? format(field.value, "dd MMM yyyy", { locale: fr }) : <span>Sélectionner...</span>}
                    ...
                    <PopoverContent className="w-auto p-0 rounded-2xl">
                      <Calendar mode="single" selected={field.value} onSelect={field.onChange} captionLayout="dropdown-buttons" fromYear={2000} toYear={new Date().getFullYear()} />
                    </PopoverContent>
                  </Popover>
                  <FormMessage />
                </FormItem>
        ...
        {isVerifying && (
          <div className="p-4 rounded-2xl border border-indigo-500/25 bg-indigo-500/5 text-indigo-300 text-xs flex items-start gap-2.5 mb-2 animate-pulse">
            ...
            <div>
              ...
              <p className="mt-0.5 text-muted-foreground leading-relaxed text-[11px]">Notre système est en train de vérifier la validité du document, d'extraire les dates et de confirmer la correspondance de votre identité. Veuillez patienter quelques instants...</p>
            </div>
          </div>
        )}
        ...
        {!isVerifying && docInfo?.status === 'rejected' && docInfo.rejectionReason && (
          <div className="p-4 rounded-2xl border border-rose-500/25 bg-rose-500/5 text-rose-300 text-xs flex items-start gap-2.5 mb-2">
            ...
            <div className="flex-1">
              <p className="font-bold text-foreground">Document refusé par notre système</p>
              <p className="mt-0.5 text-muted-foreground leading-relaxed text-[11px]">{docInfo.rejectionReason}</p>
              <p className="mt-1 text-[10px] text-rose-400/80 font-medium">Veuillez corriger le document et le téléverser de nouveau pour relancer la vérification.</p>
              <div className="mt-3">
                <Button 
                  type="button" 
                ...
                >
                  Demander une vérification manuelle par un admin
                </Button>
              </div>
            </div>
          </div>
        ...
        {!isVerifying && docInfo?.status === 'manual_verification' && (
          <div className="p-4 rounded-2xl border border-blue-500/25 bg-blue-500/5 text-blue-300 text-xs flex items-start gap-2.5 mb-2">
            ...
            <div>
              <p className="font-bold text-foreground">Vérification manuelle en cours</p>
              <p className="mt-0.5 text-muted-foreground leading-relaxed text-[11px]">Votre demande a été transmise à un administrateur général pour vérification manuelle. Vous recevrez une notification dès que vos documents auront été vérifiés.</p>
            </div>
          </div>
        )}
        ...
        {!isVerifying && docInfo?.status && (
          <div className="p-3.5 rounded-2xl border border-border/10 bg-muted/30 dark:bg-slate-900/10 space-y-2">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Résultats de la vérification :</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="flex items-center gap-1.5 p-2 rounded-xl bg-background border border-border/20 text-xs text-muted-foreground">
                ...
                <span className="truncate">Type de Doc</span>
              </div>
              <div className="flex items-center gap-1.5 p-2 rounded-xl bg-background border border-border/20 text-xs text-muted-foreground">
                {docInfo.status === 'approved' ? (
                  <CheckCircle2 className="h-4 w-4 text-emerald-400 shrink-0" />
                ...
                <span className="truncate">Nom & Prénom</span>
              </div>
              <div className="flex items-center gap-1.5 p-2 rounded-xl bg-background border border-border/20 text-xs text-muted-foreground">
                {expWarning?.type === 'expired' ? (
                  <XCircle className="h-4 w-4 text-rose-400 shrink-0" />
                ...
                <span className="truncate">Date Validité</span>
              </div>
            </div>
            ...
            {docInfo.warnings && docInfo.warnings.length > 0 && (
              <div className="mt-2 p-2 rounded-xl bg-amber-500/5 border border-amber-500/20 text-xs text-amber-300 space-y-1">
                <p className="font-bold flex items-center gap-1">
                  <AlertCircle size={12} className="shrink-0" /> Alertes détectées :
                </p>
        ...
        {!isInfoSaved ? (
          <div className="p-6 rounded-3xl border-2 border-dashed border-rose-500/25 bg-rose-500/5 backdrop-blur-sm min-h-[140px] flex flex-col items-center justify-center gap-3 text-center">
            ...
            <div>
              <p className="text-xs font-bold text-foreground">Téléversement verrouillé</p>
              <p className="text-[10px] text-muted-foreground mt-1 max-w-xs mx-auto">
                Veuillez renseigner le numéro et les dates de validité ci-dessus, puis cliquez sur <strong>« Enregistrer les infos »</strong> pour déverrouiller le téléversement.
              </p>
            </div>
          </div>
        ) : (
          ...
          {isUploading ? (
            <div className="w-full space-y-3 py-2 text-center">
              ...
              <div>
                <p className="text-xs font-bold text-foreground">Téléversement en cours...</p>
                <p className="text-[10px] text-muted-foreground mt-0.5">Veuillez ne pas fermer cette page.</p>
              </div>
            ...
            <>
              ...
              <div className="text-center">
                <p className="text-xs font-bold text-foreground">
                  {hasUploaded ? "Glissez un fichier ou cliquez pour remplacer" : "Glissez-déposez votre document ici"}
                </p>
                <p className="text-[10px] text-muted-foreground mt-1">
                  PDF, JPEG ou PNG jusqu'à 5 Mo
                </p>
              </div>
            </>
          )}
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-bold text-foreground truncate max-w-[150px] sm:max-w-[250px]">
                  {docInfo.fileName?.split('-').slice(2).join('-') || "Document téléversé"}
                </p>
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  Téléversé le : {docInfo.uploadedAt ? format(docInfo.uploadedAt.toDate(), "dd MMM yyyy 'à' HH:mm", { locale: fr }) : 'N/A'}
                </p>
              </div>
            </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border/40 pb-5">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Documents de Vérification</h1>
          <p className="text-sm text-muted-foreground mt-1">Fournissez vos pièces justificatives pour faire valider officiellement votre profil.</p>
        </div>
      </div>
        ...
        <CardHeader className="flex flex-row items-center gap-4">
          ...
          <div>
            ...
            <CardDescription className="text-xs mt-1">
              {isVerified 
                ? "Félicitations, vos documents ont été approuvés par l'administration. Vous pouvez postuler aux offres." 
                : "Vous devez téléverser les documents requis ci-dessous pour que l'administration puisse valider votre profil."
              }
            </CardDescription>
          </div>
        </CardHeader>
