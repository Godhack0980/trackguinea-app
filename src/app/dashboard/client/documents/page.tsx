...
import { Loader2, FileUp, Trash2, Eye, Calendar as CalendarIcon, Save, AlertTriangle } from "lucide-react";
...
const DocumentManager = ({ 
  ...
  const docKey = 'identityCard'; // Document key for clients is identityCard
  ...
  const handleDeleteFile = async () => {
    ...
    try {
        ...
        await updateDoc(userDocRef, {
            [`documents.${docKey}`]: {}, // Clear the whole object
        });
  ...
  return (
    <Card className="shadow-md rounded-2xl border-border">
      <CardHeader>
        <CardTitle className="text-lg text-accent">Pièce d'identité (Carte Nationale ou Passeport)</CardTitle>
        <CardDescription>Renseignez les informations et téléversez une copie du document pour faire vérifier votre compte.</CardDescription>
      </CardHeader>
...
export default function ClientDocumentsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-3xl font-bold text-primary">Mes Documents</h1>
      {userData?.isVerified === false && (
          <Card className="border-destructive shadow-md rounded-2xl">
             <CardHeader>
                <CardTitle className="flex items-center gap-2 text-destructive"><AlertTriangle/>Compte en attente de vérification</CardTitle>
                <CardDescription>
                    Pour utiliser la plateforme, votre compte doit être vérifié par un administrateur. Veuillez fournir les documents requis ci-dessous pour accélérer le processus.
                </CardDescription>
            </CardHeader>
          </Card>
      )}
